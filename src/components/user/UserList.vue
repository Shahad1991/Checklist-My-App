<template>
  ...
  <v-container :style="{ width: '100%', padding: '25px' }" class="userliste">
    <v-col cols="12" class="text-center">
      <h2 class="question">Alle bidrager til en grønnere fremtid</h2>
    </v-col>
    <br/><br/>
      ...
      <v-list :style="{backgroundColor:'#3E7A00'}">
        <v-list-item v-for="(user, index) in users" :key="user.user_id" class="user-card">
          <v-card :class="{ 'highest-points': index === 0 }">
            ...
            <v-card-subtitle>Level: {{ user.level }} | Optjente dråber: {{ user.points }}</v-card-subtitle>
          </v-card>
        </v-list-item>
      </v-list>
    </v-col>
    <br/><br/><br/><br/>
    <v-col cols="auto" class="navigation-col" @click="tilbage" style="display: flex; align-items: center;">
      <img src="@/Images/arrow left.svg" alt="Arrow left" style="width: 30px; cursor: pointer; margin-right: 10px;"/>
...
.highest-points {
  background-color: #D3BE03; /* Guld farve for brugeren med de højeste point */
  border-style: solid;
}
